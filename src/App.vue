<template>
  <div class="app">
    ...
    <div id="operator">
      <button v-show="!recording" @click="startRecording">开始录音</button>
      <button v-show="recording" @click="stopRecording">停止录音</button>
...
export default {
  ...
  data() {
    return {
      ...
      uri: "ws://192.168.1.16:8080/camera/uploader/ws", // 科大讯飞地址
      result: "", 
      recording: false, 
      sessionID: "",
  ...
  methods: {
    ...
    init() {
      ...
      this.state = "init";
      let hostPort = process.env.VUE_APP_API;
    ...
    createWebSocket() {
      ...
      console.log("create now api:", url)
      console.log(url);
    ...
    processWsMessage(e) {
      ...
      console.log("websocket rev:",e.data)
      if (jsonData.action == "started") {
        console.log("握手成功");
      } else if (jsonData.topic == "asr.ing") {
        // this.setResult(JSON.parse(jsonData.nlg));
        let asr = jsonData.text;
        this.addChatMessage(asr, "sent");
      } else if (jsonData.topic == "dm.result") {
        // this.setResult(JSON.parse(jsonData.nlg));
        let nlg = jsonData.dm.nlg;
        // 新开一行
    ...
    setResult(data) {
      ...
      //this.addChatMessage("我: " + nlg, "sent");
